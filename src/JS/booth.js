 ...
 */
if (!utils.getConfig().triggers || utils.getConfig().triggers.onClick) {
  /*  $("body").click(function () {
     trigger();
   }); */
}
 ...
 */
if (
  utils.getConfig().init.useGPIO !== undefined
    ? utils.getConfig().init.useGPIO
    : true
) {
...
const firstPhotoCountdownLength = utils.getConfig().firstPhotoCountdownLength
  ? Number(utils.getConfig().firstPhotoCountdownLength)
  : 5;
const followingPhotosCountdownLength = utils.getConfig()
  .followingPhotosCountdownLength
  ? Number(utils.getConfig().followingPhotosCountdownLength)
  : 3;
const photoPreviewDuration = utils.getConfig().photoPreviewDuration
  ? Number(utils.getConfig().photoPreviewDuration)
  : 8;
const photoSeriesLength = utils.getConfig().photoSeriesLength
  ? Number(utils.getConfig().photoSeriesLength)
  : 3;
...
function trigger(callback) {
  ...
  if (camera.isInitialized()) {
    ...
    let prompt = new CountdownPrompt(countdownLength).start(
      true,
      false,
      function () {
    ...
    setTimeout(function () {
      if (livePreview) livePreview.stop();
      if (
        utils.getConfig().flash !== undefined &&
        utils.getConfig().flash.enabled
      ) {
        /* const flash = $("#flash");
      ...
      camera.takePicture(function (res, msg1, msg2) {
        ...
        prompt.stop(true, false, function () {
          ...
          if (res === 0) {
            ...
            prompt = new PreviewPrompt(message1, photoPreviewDuration).start(
              false,
              false,
              function () {
            ...
            if (livePreview) livePreview.start();
            slideshow.start();
          } else {
            ...
            if (res === -1) {
              ...
              // sharp error
              new SharpErrorPrompt(5).start(false, false, function () {
                executing = false;
              });
            }
          }
