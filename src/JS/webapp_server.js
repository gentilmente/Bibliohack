...
process.on("uncaughtException", function (err) {
  if (err.errno === "EACCES") {
    console.warn(
      "webapp: photo-booth must be run as root to use port 80. " +
...
io.on("connection", function (socket) {
  ...
  socket.on("contact address", function (msg) {
    var contentDir = utils.getContentDirectory();
    fs.appendFile(
      contentDir + "/contact-addresses.txt",
      msg + ",\n",
      function (err) {
        if (err) {
          ...
          console.log(
            "webapp:",
            "'" + msg + "'",
            "added to contact-addresses.txt"
          );
        }
      }
    );
      ...
      if (files) {
        ...
        for (var i = 0; i < files.length; i++) {
          var isJpeg =
            ...
            files[i].endsWith(".JPG") ||
            files[i].endsWith(".JPEG");
          if (!files[i].includes("large") && isJpeg) {
            // filter unconverted photos
            images.push("photos/" + files[i]);
          }
        }

        console.log(
          "webapp: sending " + files.length + " latest photos to webapp"
        );
  ...
  socket.on("get_download_image", function (path, grayscale) {
    ...
    utils.convertImageForDownload(
      filename,
      grayscale,
      function (res, path, err) {
        if (res) {
  ...
  socket.on("trigger_photo", function (password) {
    ...
    if (
      utils.getConfig().webapp.enableRemoteRelease ||
  ...
  socket.on("print_preview", function (layout, images) {
    const paths = images
      .map((file) => file.substr(file.indexOf("/") + 1))
  ...
  socket.on("print", function (layout, images, printCount, password) {
    ...
    if (
      printCount >= utils.getConfig().printing.limitPerUser &&
      utils.getConfig().printing.limitPerUser > 0 &&
      !passwordIsValid(password)
    ) {
