...
class Collage {
  ...
  getPlaceholderImage(layout, callback) {
    ...
    const tempFile = path.join(
      utils.getTempDir(),
      layout + "_" + utils.getTimestamp() + ".jpeg"
    );
    const that = this;
    sharp({
      create: {
  ...
  createCollage(layout, images, callback) {
    ...
    const convertedFilepath = path.join(
      utils.getFullSizePhotosDirectory(),
      newFilename
    );
  ...
  _createCollage(filePath, options, images, callback) {
    const layoutAsBase64 = new Buffer.from(JSON.stringify(options)).toString(
      "base64"
    );
    const optionsAsBase64 = new Buffer.from(
      JSON.stringify({
