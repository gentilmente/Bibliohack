...
class Utils {
  ...
  saveConfig(new_config, callback) {
    ...
    fs.writeFile(
      this.config_path,
      JSON.stringify(new_config, null, "\t"),
      function (err) {
        if (err) {
  ...
  getContentDirectory() {
    if (this.contentDir === undefined) {
      ...
      if (
        newContentDir !== null &&
        typeof newContentDir === "string" &&
        newContentDir.length > 0
      ) {
        // if valid path in config
        try {
          if (!fs.existsSync(String(newContentDir)))
            fs.mkdirSync(String(newContentDir));
          this.contentDir = newContentDir.startsWith("/")
            ? newContentDir
            : path.join(__dirname, "../", newContentDir);
        } catch (err) {
          // fallback: default
          console.error(
            "Could not open or create content_dir '" +
              this.config.content_dir +
              "' like defined in config.json. " +
              err +
              "\nInstead going to use default '",
            this.defaultContentDirectory,
            "'"
          );
          if (!fs.existsSync(this.defaultContentDirectory))
            fs.mkdirSync(this.defaultContentDirectory);
  ...
  loadRecentImagesAfterStart() {
    ...
    fs.readdir(photos_dir, function (err, files) {
      if (files) {
        files.sort();
        var numberImages =
          files.length < self.maxImages ? files.length : self.maxImages;
  ...
  createGifForDownload(filenames, grayscale, callback) {
    ...
    try {
      this._processAndAddToGif(
        gifCallback,
        filenames,
        grayscale,
        function (res, message, err) {
          gif.finish();
  ...
  _processImageInternal(filename, grayscale, callback) {
    ...
    var converter = sharp(_path).resize(
      this.config.webapp.maxDownloadImageSize
    );
