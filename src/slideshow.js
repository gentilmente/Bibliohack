...
class Slideshow {
  constructor() {
    if (
      utils.getConfig().slideshow !== undefined &&
      utils.getConfig().slideshow.enabled
    ) {
      var params = utils.getConfig().slideshow;

      this.enabled = true;

      this.delay = params.activatesAfterSeconds
        ? Number(params.activatesAfterSeconds)
        : 30;
      this.duration = params.secondsPerPhoto
        ? Number(params.secondsPerPhoto)
        : 8;
      ...
      self.slideshowDelay = setTimeout(function () {
        self.slideshow = setInterval(function () {
          ...
          $("#collage").append(
            "<div id='slideshow-" +
              clzz +
              "' class='slideshow' style='background-image: url(" +
              arr[idx] +
              ");'>"
          );
