...
const gphoto2 = utils.getConfig().gphoto2.simulate ? null : require("gphoto2");
...
class Camera {
  ...
  initialize(callback) {
    ...
    this.GPhoto.list(function (list) {
      ...
      if (utils.getConfig().gphoto2.capturetarget) {
        self.camera.setConfigValue(
          "capturetarget",
          utils.getConfig().gphoto2.capturetarget,
          function (err) {
            if (err) {
  ...
  _createSamplePicture(callback) {
    ...
    const watermark = new Buffer.from(`<svg width="3000" height="2000">
  ...
  _resizeAndSave(data, callback) {
    ...
    function resizeInternal() {
      ...
      const maxImageSize = utils.getConfig().maxImageSize
        ? utils.getConfig().maxImageSize
        : 1500;
